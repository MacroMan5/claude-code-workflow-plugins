...
def is_directory_scan_blocked(command: str) -> tuple[bool, str]:
    ...
    is_scan_command = any(pattern.search(normalized) for pattern in SCAN_COMMAND_PATTERNS)
    ...
    if FIND_CWD_PATTERN.search(normalized):  # find . (current directory recursively)
        ...
        excludes_blocked = any(
            f'-not -path "*/{blocked_dir}/*"' in command or
            f'-prune -name {blocked_dir}' in command
            for blocked_dir in blocked_dirs
        )
...
def main():
    ...
    try:
        ...
        if tool_name == 'Bash':
            command = tool_input.get('command', '')
            normalized = ' '.join(command.lower().split())
            if 'sudo ' in normalized and os.getenv('LAZYDEV_ALLOW_SUDO') not in {'1','true','TRUE'}:
                print("BLOCKED: sudo is disabled for safety (set LAZYDEV_ALLOW_SUDO=1 to allow)", file=sys.stderr)
                sys.exit(2)
            ...
            if is_force_push_to_main(command):
                ...
                print(
                    "This prevents rewriting production history",
                    file=sys.stderr
                )
                sys.exit(2)
